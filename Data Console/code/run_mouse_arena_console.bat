@echo off
REM Mouse Arena Data Console launcher.
REM
REM Runs from the dedicated conda-forge environment "ma-console", which has a
REM self-consistent Qt6/ICU stack (the pip PySide6 in vras clashed with
REM Anaconda's own Qt6). Create it once with:
REM
...
if exist "%ENVPY%" (
  "%ENVPY%" "run_mouse_arena_console.py" %*
) else (
  echo Could not find the ma-console env python; trying "conda run"...
  call conda run -n ma-console python "run_mouse_arena_console.py" %*
)
